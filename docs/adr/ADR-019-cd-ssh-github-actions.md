...
Côté implémentation du pas SSH lui-même, deux approches ont été essayées :

- **Action tierce `appleboy/ssh-action`, vérification d'hôte via son entrée `fingerprint`** — la plus rapide à écrire, mais en pratique, avec l'empreinte ed25519 du VPS confirmée correcte par deux méthodes indépendantes (avertissement du client OpenSSH à la première connexion, lecture directe de `/etc/ssh/ssh_host_ed25519_key.pub` sur le serveur), la vérification échouait systématiquement (`ssh: host key fingerprint mismatch`) sans piste de diagnostic exploitable sans accès direct au run.
- **SSH natif (`ssh` du runner) + `known_hosts` classique** — mécanisme standard, documenté, sans dépendre du fonctionnement interne d'une action tierce pour un point aussi sensible que la vérification d'hôte.
...
La connexion SSH elle-même se fait en client natif : le job écrit la clé privée (`VPS_DEPLOY_SSH_KEY`) et un `known_hosts` pré-rempli (secret `VPS_HOST_KEY`, la ligne complète `<host> ssh-ed25519 <clé-publique-du-VPS>`) avant d'appeler `ssh -o StrictHostKeyChecking=yes`, plutôt que de passer par `appleboy/ssh-action`.
...
- Vérification d'hôte assurée par le client SSH natif (`known_hosts` + `StrictHostKeyChecking=yes`), pas par une action tierce — une dépendance de moins sur le chemin critique du déploiement, et un mécanisme dont le comportement est directement documenté par OpenSSH plutôt que par le code d'un projet GitHub Actions externe.
